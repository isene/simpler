...
# (variant types with match, functions, calls, locals, arithmetic, print),
# and the C it emits compiles and runs to the right value
"$SIMPLER" run ../selfhost/simpler.smplr > "$TMP/sh.c" 2>/dev/null
if cc -o "$TMP/sh" "$TMP/sh.c" 2>/dev/null && [ "$("$TMP/sh")" = "13" ]; then ok; else nope "self-hosted compiler builds its subset"; fi
